  ...
  if (activeTab === 'verification') {
    return (
      <div className="flex-1 p-6 flex flex-col gap-6 overflow-hidden text-left font-mono bg-black/40 relative">
        ...
        <div className="flex flex-col gap-4 bg-zinc-950/40 p-4 border border-white/5 rounded-sm shrink-0">
          <label className="text-[9px] text-[#00ffff]/60 font-black uppercase tracking-widest flex items-center gap-2 mb-1">
            <LayoutGrid size={12} /> Pilih Mode Verifikasi
          </label>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
            {CAPTURE_MODES.map((m) => (
              <button key={m.id} onClick={() => setMode(m.id)} className={`flex flex-col items-center justify-center py-2 px-1 border-2 rounded-sm transition-all ${mode === m.id ? 'bg-[#00ffff]/20 border-[#00ffff] text-[#00ffff] shadow-[0_0_10px_#00ffff44]' : 'bg-black/40 border-white/10 text-zinc-500 hover:border-white/30'}`}>
                <span className="text-[9px] font-black">{m.label}</span>
                <span className="text-[6px] opacity-60 uppercase">{m.detail}</span>
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            }} disabled={!isConnected || isCapturing} className={`flex-1 py-3 border font-black text-[10px] uppercase transition-all flex items-center justify-center gap-2 rounded-sm ${isConnected && !isCapturing ? 'bg-[#00ffff]/10 border-[#00ffff] text-[#00ffff] hover:bg-[#00ffff]' : 'bg-zinc-900 border-zinc-800 text-zinc-600'}`}><Play size={12} fill="currentColor" /> Preview</button>
            <button onClick={async () => {
              const res = await handleAction('/api/fingerprint/stopcapture');
              if (res.success !== false) setIsCapturing(false);
            }} disabled={!isCapturing} className={`flex-1 py-3 border font-black text-[10px] uppercase transition-all flex items-center justify-center gap-2 rounded-sm ${isCapturing ? 'bg-rose-500/10 border-rose-500 text-rose-500 hover:bg-rose-500 hover:text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-600'}`}><CircleStop size={12} fill="currentColor" /> Stop</button>
            <button onClick={handleVerifyLogic} disabled={!isConnected || isVerifying} className={`flex-[1.5] py-3 border font-black text-[10px] uppercase transition-all flex items-center justify-center gap-2 rounded-sm ${isConnected ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-lg' : 'bg-zinc-900 border-zinc-800 text-zinc-600'}`}>{isVerifying ? <Loader2 size={12} className="animate-spin" /> : <ShieldCheck size={12} />} Verify</button>
          </div>
        </div>
  ...
  return (
    <div className="flex-1 p-4 flex flex-col gap-4 overflow-y-auto custom-scrollbar text-left font-mono relative">
            ...
            <div className="flex flex-col gap-2 mt-auto">
              <div className="flex gap-2">
                <button onClick={async () => {
                  const res = await handleAction('/api/fingerprint/startcapture', { mode: parseInt(mode), nMissingFinger: 0 });
                  if (res.success) setIsCapturing(true);
                }} disabled={!isConnected || isCapturing} className={`flex-1 py-2 border-2 font-black uppercase text-[9px] transition-all flex items-center justify-center gap-2 rounded-sm ${isConnected && !isCapturing ? 'bg-[#00ffff]/10 border-[#00ffff] text-[#00ffff] hover:bg-[#00ffff] hover:text-black shadow-inner' : 'bg-zinc-900 border-zinc-800 text-zinc-600 cursor-not-allowed'}`}><Play size={12} fill="currentColor" /> Preview</button>
                <button onClick={async () => {
                  const res = await handleAction('/api/fingerprint/stopcapture');
                  if (res.success !== false) setIsCapturing(false);
                }} disabled={!isCapturing} className={`flex-1 py-2 border-2 font-black uppercase text-[9px] transition-all flex items-center justify-center gap-2 rounded-sm ${isCapturing ? 'bg-zinc-950 border-[#ff00ff] text-[#ff00ff] hover:bg-[#ff00ff] hover:text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-600'}`}><CircleStop size={12} fill="currentColor" /> Stop</button>
              </div>
              <div className="flex gap-2">
                <button onClick={handleEnroll} disabled={!isConnected || isEnrolling || !isDataSaved || enrollmentComplete} className={`flex-[2] py-3 border-2 font-black text-[10px] uppercase transition-all rounded-sm flex items-center justify-center gap-2 ${isConnected && isDataSaved && !isEnrolling && !enrollmentComplete ? 'bg-[#00ffff] border-[#00ffff] text-black hover:bg-white shadow-lg' : 'bg-zinc-900 border-zinc-700 text-zinc-600'}`}><ShieldCheck size={14} /> {enrollmentComplete ? "SELESAI" : "Start Enroll"}</button>
                <button onClick={handleResetForm} className="flex-1 py-3 border-2 border-red-500/30 text-red-500 text-[10px] font-black hover:bg-red-500 hover:text-white uppercase transition-all rounded-sm flex items-center justify-center gap-2"><Trash2 size={12} /> Reset</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full border-2 border-[#00ffff]/20 bg-black/40 p-5 rounded-sm shadow-2xl shrink-0 flex-1 min-h-0 flex flex-col">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-10 gap-3 flex-1 overflow-y-auto custom-scrollbar p-1">
